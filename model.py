...
class InTraModel(nn.Module):
    def __init__(
        ...
        """
        grid_size_max:
        patch_size: This is the desired side length of a square patch. Called patch_size in the paper. (following ViT, patch_size=16)
        window_size: This the side length of a subgrid in the full grid that is used as context window. Called window_size in the paper.
        ...
        # Project (patch_size*patch_size*C) dim patch into D dim embedding
        self.linear_projection = nn.Linear(num_channels * (patch_size ** 2), embed_dim)
        # Project D dim embedding into (patch_size*patch_size*C) dim patch.
    ...
    def forward(self, x_nbr, patch_positions, inpaint_position):
        """
        Let's say, if window size window_size=7, patch size patch_size=16, channel=3,
                   (window_size*window_size)-1 --> 48;  (C*patch_size*patch_size) --> 768
        Args:
        ...
        # linear projection from (C*patch_size*patch_size) to D dimensional embedding
        ...
        # Create (window_size.window_size) * D dimensional embedding for transformer block
        x = torch.cat([x_inp, x_nbr], dim=1)

        x = self.encoder(x)

        # [B*(window_size*window_size)*D] dim to [B, 1, D] dim by averaging.
        x = torch.mean(x, dim=1)

        # Project back to patch dimension(flatten state)
        # [B, 1, D] --> [B, (patch_size*patch_size*C)]
...
class InTra(pl.LightningModule):
    ...
    def _step(self, batch, mode):
        ...
        X = patches_info["context_patches"]
        Y = patches_info["target_patch"]
        X_positions = patches_info["context_positions"]
        Y_position = patches_info["target_position"]
